...
import { Input } from './ui/input'; // For Remarks/Condition if needed
...
export function AssignAssetModal({ isOpen, onClose, onSuccess, assetId, assetName }: AssignAssetModalProps) {
    ...
    const onSubmit = async (data: AssignFormValues) => {
        setIsLoading(true);
        try {
            await AssetService.assignAsset(assetId, data.employee_id, {
                condition: data.condition,
                remarks: data.remarks,
                // assignedBy: 'Admin' // Removed to avoid UUID error since we don't have auth user ID handy and schema expects UUID
            });
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[425px]" ref={modalRef}>
                <DialogHeader>
                    <DialogTitle>Assign Asset: {assetName}</DialogTitle>
                </DialogHeader>
